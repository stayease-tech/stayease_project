...
function BedsTable() {
    ...
    const updateBedsDataHandle = (bedsData, status) => {
        status === 'Pending'
            ? navigate(`/operations/operations-moveinchecklist-form/${bedsData?.resident_data?.id}`, { state: { bedsData } })
            : navigate(`/operations/operations-checklistfeedback-table`, { state: { bedsData } });
    };
                    ...
                    <table className="min-w-full table-auto text-xs border-collapse">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200">
                                ...
                                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wide whitespace-nowrap">Room</th>
                                ...
                                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wide whitespace-nowrap">Resident</th>
                                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wide whitespace-nowrap">Move-In Status</th>
                                ...
                                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wide whitespace-nowrap">Complaint Link</th>
                            </tr>
                        </thead>
                            ...
                            {loadingData ? (
                                ...
                                paginatedData.map((bedData, i) => (
                                    <tr className="hover:bg-gray-50 transition-colors" key={bedData.id}>
                                        ...
                                        <td className="px-3 py-1.5 text-xs text-gray-800 max-w-[200px] truncate">{bedData?.propertyName}</td>
                                        ...
                                        <td className="px-3 py-1.5 text-xs text-gray-800 max-w-[200px] truncate">{bedData?.resident_data?.residentsName}</td>
                                        <td className="px-3 py-1.5 text-xs text-gray-800">
                                            <div className="flex items-center gap-1.5">
                                                <span className={moveInStatusBadge(bedData?.resident_data?.moveInChecklistStatus)}>
                                                ...
                                                {bedData?.resident_data?.moveInChecklistStatus === 'Pending' ? (
                                                    <PlusCircle
                                                        ...
                                                        onClick={() => updateBedsDataHandle(bedData, 'Pending')}
                                                    />
                                                ) : (
                                                    <Eye
                                                        size={14}
                                                        className="text-gray-400 hover:text-[#D4A017] cursor-pointer transition-colors"
                                                        onClick={() => updateBedsDataHandle(bedData, 'Completed')}
                                                    />
                                                )}
                                            </div>
                                        </td>
                                ...
                                <tr>
                                    <td colSpan={9} className="px-3 py-10 text-center text-gray-400">No data available</td>
                                </tr>
                            )}
                        </tbody>
                    </table>

...
function ActivityStats({ isExpanded, setIsExpanded }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await axios.get('/operations/get-user-activity-data/');
    ...
    const viewLoginDataTable = (user) => {
        navigate(`/operations/operations-login-data/${user?.id}`);
    };
                                        ...
                                        <tr className="" key={user.id}>
                                            ...
                                            <td className="border border-gray-300 px-4 py-2 text-center">
                                                <div className="flex justify-center">
                                                    <FaEye className="hover:text-[#D4A017] text-xl hover:cursor-pointer block" onClick={() => viewLoginDataTable(user)} />
                                                </div>
                                            </td>
                                        </tr>
                                    )) : <tr>

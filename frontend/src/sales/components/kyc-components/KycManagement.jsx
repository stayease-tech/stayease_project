          ...
          <div className="divide-y divide-gray-100">
            {paged.map((t) => (
              <div
                ...
                onClick={() =>
                  navigate(`/sales/sales-kyc-management/${t.id}`, {
                    state: { resident: t, filter },
                  })
                }
              >
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs border ${STATUS_BADGE[t.kycApprovalStatus] || ''}`}
                      >
                        {t.kycApprovalStatus}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 mt-0.5">
                      {t.phoneNumber} &bull; {t.email} &bull; Room {t.roomNo}
                    </p>
                  </div>
                </div>
                <ChevronRight

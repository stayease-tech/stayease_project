...
function BedsTable() {
  ...
  const viewAgreementHandle = (bedData) => {
    return navigate(
      `/sales/sales-agreement-pdf/${bedData?.resident_data?.id}`,
      { state: { bedData } }
    );
  };
  ...
  const viewresidentsDataHandle = (bedData) => {
    if (
      bedData?.resident_data &&
      Object.keys(bedData.resident_data).length > 0
    ) {
      const d = bedsData.filter((item) => item.id === bedData.id);
      return navigate(`/sales/sales-residents-table/${bedData?.id}`, {
        state: { bedsData: d },
      });
    }
        ...
        >
          ...
          {getOptions('sales_statuses').map((s, i) => (
            <option
              key={i}
              value={s}
            >{`${s} (${data.filter((bed) => bed.salesStatus === s).length})`}</option>
          ))}
        </select>
      </div>
          ...
          <table className="min-w-full table-auto text-xs border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loadingData ? (
                ...
                paginatedData.map((bed, i) => {
                  const rentStatus =
                    bed?.salesStatus === 'Completed' &&
                    bed?.resident_data?.residentStatus === 'Active' &&
                    bed?.resident_data?.rent_records?.length > 0
                      ? bed.resident_data.rent_records[
                          bed.resident_data.rent_records.length - 1
                        ]?.rentStatus
                      : 'NA';
                  return (
                    <tr
                    ...
                    >
                        ...
                        <Eye
                          ...
                          onClick={() =>
                            bed.salesStatus === 'Pending'
                              ? alert(
                                  'Currently there is no resident allocated to generate the agreement!'
                                )
                      ...
                      <td className="px-3 py-1.5 text-xs text-gray-800">
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs ${rentStatusColors[rentStatus] || 'bg-gray-100 text-gray-600'}`}
                        >
                          {rentStatus}
                        </span>
                      </td>

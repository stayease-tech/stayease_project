    ...
    return (
        ...
        <DashPage>
                        ...
                        <div className="sm:flex justify-between">
                            <button
                                className="block max-sm:w-full mb-5 px-4 py-2 bg-[#D4A017] text-white text-base font-medium rounded cursor-pointer hover:bg-[#B8860B] max-sm:text-sm" type="button"
                                onClick={() => {
                                    if (type === 'BedsTable') {
                                        navigate('/accounts/accounts-beds-table');
                                    }
                                    if (type === 'BedsDetails') {
                                        navigate(`/accounts/accounts-beds-details/${bedData?.id}`, { state: { bedData } });
                                    }
                                    if (type === 'LiabilityTable') {
                                        navigate('/accounts/accounts-liability-table');
                                    }
                                    if (type === 'LiabilityData') {
                                        navigate(`/accounts/accounts-liability-data/${bedData?.id}`, { state: { bedData } });
                                    }
                                }}>Prev</button>

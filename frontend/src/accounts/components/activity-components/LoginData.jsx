        ...
        <DashPage>
            ...
            <div className="flex items-center mb-3">
                <button
                    ...
                    onClick={() => navigate(`/accounts/accounts-user-activity-data`)}
                    type="button"
                >
                    Prev
                </button>
                                    ...
                                    {paginatedData.map((user, i) => (
                                        <tr className="hover:bg-gray-50 transition-colors" key={user.id}>
                                            ...
                                            <td className="px-3 py-1.5 text-xs text-gray-800">
                                                {`${calculateLoginDuration(user?.login_time, user?.logout_time)} ${(userLoginData[userLoginData.length - 1].login_time === user.login_time && user?.logout_time === null) ? '(live)' : ''}`}
                                            </td>
                                        </tr>
                                    ))}
                                    <tr className="bg-gray-50 font-medium">
                                        <td colSpan="3" className="px-3 py-1.5 text-xs text-gray-700 text-right">Total Time Logged In</td>
                                        <td className="px-3 py-1.5 text-xs text-gray-800">{calculateTotalDuration(userLoginData)}</td>
                                    </tr>
                                </>
                            ) : (
                                <tr className="hover:bg-gray-50 transition-colors">

...
function ActivityStats() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await axios.get('/accounts/get-user-activity-data/');
    ...
    const viewLoginDataTable = (user) => {
        navigate(`/accounts/accounts-login-data/${user?.id}`);
    };

    return (
        <DashPage>
            <h1 className="text-center sm:text-xl lg:text-2xl font-semibold lg:mt-0 mb-8 text-[#D4A017]">USER ACTIVITY TABLE</h1>
            ...
            <div className="sm:flex justify-end">
                <input
                    ...
                    className="block mt-2 mb-3 text-black max-sm:w-full p-2 border border-gray-300 rounded text-sm placeholder-gray-400 placeholder:text-xs"
                />
            </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {paginatedData.length > 0 ? paginatedData.map((user, i) => (
                            <tr className="hover:bg-gray-50 transition-colors" key={user.id}>
                                ...
                                <td className="px-3 py-1.5 text-xs text-gray-800 max-w-[180px] truncate">{user?.username}</td>
                                <td className="px-3 py-1.5 text-xs text-gray-800 max-w-[180px] truncate">{user?.useremail}</td>

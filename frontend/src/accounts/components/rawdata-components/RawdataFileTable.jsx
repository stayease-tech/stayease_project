...
function RawdataFileTable({ isExpanded, setIsExpanded }) {
                            ...
                            <table className="min-w-full table-auto border-collapse shadow-md rounded-lg max-sm:text-xs">
                                <thead>
                                    <tr className="bg-gray-50 text-gray-700">
                                        ...
                                        <th className="border border-gray-300 py-2 px-4 text-left border-b text-center">Rawdata File</th>
                                ...
                                <tbody>
                                    {paginatedData.length > 0 ? paginatedData.map((file, i) => (
                                        <tr className="" key={file.id}>
                                            ...
                                            <td className="border border-gray-300 px-4 py-2 text-center hover:text-[#D4A017] hover:cursor-pointer" onClick={() => viewRawdataFile(file?.rawdataFile)}>{file?.rawdataFile.split('/')[5]}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{formatter.format(new Date(file?.createdAt))}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{formatter.format(new Date(file?.updatedAt))}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">
                                                <div className="flex justify-evenly">
                                                    <FaEye className="hover:text-[#D4A017] text-xl hover:cursor-pointer block" onClick={() => viewRawdataTable(file)} />

                                                    <MdDelete className="hover:text-[#D4A017] text-xl hover:cursor-pointer block" type='submit' onClick={() => deleteRawdataFile(file)} />
                                                </div>
                                            </td>
                                        </tr>
                                    )) : <tr>

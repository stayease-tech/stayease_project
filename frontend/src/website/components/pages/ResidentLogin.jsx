...
export default function residentLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await auth.loginresident(formData.phone, formData.password);
    if (result.success) {
  ...
  return (
    <section className="h-dvh w-dvw bg-[#0A0A0A] flex items-center justify-center p-[4vw]">
      <div className="w-full sm:w-[min(90vw,28rem)]">
        <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl shadow-black/20 border border-white/20 overflow-hidden">
          <div className="text-center pt-[4vh] pb-[2vh] px-[5vw]">
            <div className="w-14 h-14 mx-auto mb-[2vh] bg-[#FDF6E3] text-[#D4A017] rounded-xl flex items-center justify-center shadow-lg shadow-[#D4A017]/20">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
            </div>
            <h1 className="text-xl font-bold text-gray-900">Resident Login</h1>
            <p className="text-sm text-gray-500 mt-1">Access your StayEase resident portal</p>
          </div>

          {error && (
            <div className="mx-[5vw] mt-2 bg-red-50 border border-red-200 text-red-700 px-4 py-2.5 text-sm rounded-lg">
              {error}
            </div>
          )}

          <form className="px-[5vw] sm:px-8 pt-[2vh] pb-[4vh]" onSubmit={handleSubmit}>

...
const FeaturedProperties = memo(({ marginTop, heading, properties }) => {
  ...
  const settings = useMemo(
    () => ({
      ...
      customPaging: (i) => (
        <div
          className={`mt-3 w-2 h-2 rounded-full transition-all duration-300 ${i === Math.floor(activeSlide / slidesToScroll) ? 'bg-[#eba312]' : 'bg-white'}`}
        />
      ),
  ...
  return (
    <div className={containerClass}>
      ...
      {/* Grid View */}
      <div
        className={`${marginTop ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 px-3 md:px-5' : 'hidden'}`}
      >
        {properties.map((property, index) => (
          <GridPropertyCard
            key={property.id}

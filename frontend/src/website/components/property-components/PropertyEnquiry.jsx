...
const PriceBoard = memo(({ priceBoardData }) => (
    <>
        {priceBoardData.map(data =>
            <div key={data.id} className='flex justify-between p-3 border border-[#eba312] mt-3'>
...
const EnquiryBoard = memo(({ formData, isSubmitting, handleChange, handleSubmit }) => (
    <>
...
const PropertyEnquiry = memo(({ propertyData }) => {
    ...
    const plusSlides = useCallback((n) => {
        setSlideIndex((prev) => ((prev - 1 + n + propertyData?.propertyImages.length) % propertyData?.propertyImages.length) + 1)
    ...
    const handleSubmit = useCallback(async (e) => {
        ...
        try {
            ...
            setFormData({ name: '', phone: '', email: '', property: '', submittedAt: '' }) // Reset form on success
        } catch (error) {
                ...
                <div className='flex flex-col lg:flex-row md:space-x-10'>
                    <div className='lg:w-[55vw]'>
                        ...
                        <div className='p-3 lg:p-8 text-left rounded-lg lg:border md:border-slate-200 md:mt-0 m-5'>
                            <h3 className='text-2xl font-semibold my-3 text-[#eba312]'>{propertyData?.name} ({propertyData?.location})</h3>
                            <p>
                                {propertyData?.desc}
                            </p>
                        </div>

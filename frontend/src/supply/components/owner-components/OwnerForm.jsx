...
function OwnerForm() {
  ...
  const validatePage1 = () => {
    if (
      ...
      !/^[A-Za-z ]{2,}$/.test(ownerData.ownerName.trim())
    ) {
    ...
    if (
      ...
      !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(ownerData.ownerEmail)
    ) {
  ...
  const validateOwnerData = () => {
    ...
    if (!isValidIndianPhone(ownerData.ownerPhone))
      return 'Owner phone must be exactly 10 digits.';
    if (
      !ownerData.ownerEmail ||
      !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(ownerData.ownerEmail)
    )
    ...
    if (!/^\d{12}$/.test(ownerData.aadharNumber || ''))
      return 'Aadhaar number must be 12 digits.';
    if (!ownerData.aadharVerification)
      return 'Aadhaar verification status is required.';
    if (!/^[A-Z]{5}\d{4}[A-Z]$/.test(ownerData.panNumber || ''))
      return 'PAN must follow format: ABCDE1234F.';
    if (!ownerData.panVerification)
      return 'PAN verification status is required.';

    if (!ownerData.accountHolderName?.trim())
      return 'Account holder name is required.';
    if (!/^\d{9,18}$/.test(ownerData.accountNumber || ''))
    ...
    if (!/^[A-Z]{4}0[A-Z0-9]{6}$/.test(ownerData.ifscCode || ''))
      ...
      >
        ...
        {currentStep === 'ownerData' && (
          <>
            <OwnerData
              ownerData={ownerData}
              ownerHandleChange={ownerHandleChange}
            ...
            <button
              ...
              onClick={() => {
                if (validatePage1()) dataHandleToggle('ownerKYC');
              }}
              type="button"
            >
              Next

...
function RoomTable({ isExpanded, setIsExpanded }) {
    ...
    const viewHandle = (roomData) => {
        if (roomData.status === 'Pending') {
            navigate(`/supply/supply-room-form/${roomData?.id}`, { state: { roomData, owner_id, propertyId, roomId: id } });
        } else {
            navigate(`/supply/supply-room-details/${roomData?.id}`, { state: { roomData, owner_id, propertyId, roomId: id } });
        }
    };
            ...
            <div className="flex-1 duration-300">
                ...
                <div className={`text-slate-800 bg-white lg:bg-gray-100 min-h-screen ${isExpanded ? 'ml-16 md:ml-64' : 'ml-16'} pt-[5rem] lg:pt-[6rem] px-6 pb-5`}>
                    <div className="w-[100%] lg:w-[98%] mx-auto lg:my-8 py-8 sm:p-8 lg:p-10 lg:rounded-lg lg:bg-white text-slate-800">
                        <h1 className="text-center sm:text-xl lg:text-2xl font-semibold lg:mt-0 mb-8 text-[#D4A017]">SUPPLY ROOM TABLE</h1>

                        <div className={`sm:flex ${id !== 0 ? 'justify-between' : 'justify-end'}`}>
                            {id !== 0 &&
                                <button
                                    className="mb-5 px-4 py-2 bg-[#D4A017] text-white text-base font-medium rounded cursor-pointer hover:bg-[#B8860B] max-sm:text-sm" onClick={() => propertyId === 0 ? navigate(`/supply/supply-property-table`) : navigate(`/supply/supply-property-table/${owner_id}`)}
                            ...
                            <table className="min-w-full table-auto border-collapse shadow-md rounded-lg max-sm:text-xs">
                                <thead>
                                    <tr className="bg-gray-50 text-gray-700">
                                        ...
                                        <th className="border border-gray-300 py-2 px-4 text-left border-b text-center">Add/Update Details</th>
                                    </tr>
                                </thead>

                                <tbody>
                                    {paginatedData.length > 0 ? paginatedData.map((roomData, i) => (
                                        <tr className="" key={roomData.pk}>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{startIndex + i + 1}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{roomData.buildingLevel}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{roomData.status}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">
                                                <div className="flex justify-evenly">
                                                    {(roomData.status === 'Pending') ?
                                                        <IoMdAddCircle className="hover:text-[#D4A017] text-xl hover:cursor-pointer" onClick={() => viewHandle(roomData)} />
                                                        :
                                                        <FaEdit className="hover:text-[#D4A017] text-xl hover:cursor-pointer" onClick={() => viewHandle(roomData)} />}
                                                </div>
                                            </td>
                                        </tr>
                                    )) : <tr>

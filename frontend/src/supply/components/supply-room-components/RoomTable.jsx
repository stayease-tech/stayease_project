...
function RoomTable() {
    ...
    const viewHandle = (room) => {
        if (room.status === 'Pending') {
            navigate(`/supply/supply-room-form/${room?.id}`, { state: { roomData: room, owner_id, propertyId, roomId: id } });
        } else {
            navigate(`/supply/supply-room-details/${room?.id}`, { state: { roomData: room, owner_id, propertyId, roomId: id } });
        }
    };
                ...
                <div className="flex items-center gap-2">
                    {id !== 0 && (
                        <button
                            ...
                            onClick={() => propertyId === 0 ? navigate(`/supply/supply-property-table`) : navigate(`/supply/supply-property-table/${owner_id}`)}
                            type="button"
                        >
                            Prev
                        </button>

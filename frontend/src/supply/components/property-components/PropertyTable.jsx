...
function PropertyTable({ isExpanded, setIsExpanded }) {
    ...
    const roomViewHandle = (propertyData) => {
        navigate(`/supply/supply-room-table/${propertyData?.id}`, { state: { owner_id: propertyData.owner_id, propertyId: id } });
    };

    const editPropertyHandle = (propertyData) => {
        navigate(`/supply/supply-property-details/${propertyData?.id}`, { state: { propertyData, propertyId: id } });
    };
            ...
            <div className="flex-1 duration-300">
                ...
                <div className={`text-slate-800 bg-white lg:bg-gray-100 min-h-screen ${isExpanded ? 'ml-16 md:ml-64' : 'ml-16'} pt-[5rem] lg:pt-[6rem] px-6 pb-5`}>
                    <div className="w-[100%] lg:w-[98%] mx-auto lg:my-8 py-8 sm:p-8 lg:p-10 lg:rounded-lg lg:bg-white text-slate-800">
                        <h1 className="text-center sm:text-xl lg:text-2xl font-semibold lg:mt-0 mb-8 text-[#D4A017]">SUPPLY PROPERTY TABLE</h1>

                        <div className="sm:flex justify-between">
                            <div className="flex justify-between sm:space-x-3">
                                {id !== 0 &&
                                    <button
                                        className="mb-5 px-4 py-2 bg-[#D4A017] text-white text-base font-medium rounded cursor-pointer hover:bg-[#B8860B] max-sm:text-sm" onClick={() => navigate('/supply/supply-owner-table')}
                                        type="button">Prev</button>
                                }

                                <button
                                    className="mb-5 px-4 py-2 bg-[#D4A017] text-white text-base font-medium rounded cursor-pointer hover:bg-[#B8860B] max-sm:text-sm" onClick={() => navigate((id === 0) ? `/supply/supply-owner-table` : `/supply/supply-property-form/${id}`)}
                                    type="button">Add Property</button>
                            </div>
                            ...
                            <table className="min-w-full table-auto border-collapse shadow-md rounded-lg max-sm:text-xs">
                                <thead>
                                    <tr className="bg-gray-50 text-gray-700">
                                        ...
                                        <th className="border border-gray-300 py-2 px-4 text-left border-b text-center">Property ID</th>
                                        ...
                                        <th className="border border-gray-300 py-2 px-4 text-left border-b text-center">View Room Details</th>
                                        <th className="border border-gray-300 py-2 px-4 text-left border-b text-center">Update Property Details</th>
                                    </tr>
                                </thead>

                                <tbody>
                                    {paginatedData.length > 0 ? paginatedData.map((propertyData, i) => (
                                        <tr className="" key={propertyData.id}>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{startIndex + i + 1}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{propertyData?.ownerName}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{propertyData?.serial_number}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{propertyData?.propertyName}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{formatter.format(new Date(propertyData?.submittedDateAndTime))}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">{formatter.format(new Date(propertyData?.updatedDateAndTime))}</td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">
                                                <div className="flex justify-evenly">
                                                    <FaEye className="hover:text-[#D4A017] text-xl hover:cursor-pointer" onClick={() => roomViewHandle(propertyData)} />
                                                </div>
                                            </td>
                                            <td className="border border-gray-300 px-4 py-2 text-center">
                                                <div className="flex justify-evenly">
                                                    <FaEdit className="hover:text-[#D4A017] text-xl hover:cursor-pointer" onClick={() => editPropertyHandle(propertyData)} />
                                                </div>
                                            </td>
                                        </tr>
                                    )) : <tr>

...
function PropertyTable() {
  ...
  const roomViewHandle = (property) => {
    navigate(`/supply/supply-room-table/${property?.id}`, {
      state: { owner_id: property.owner_id, propertyId: id },
    });
  };

  const editPropertyHandle = (property) => {
    navigate(`/supply/supply-property-details/${property?.id}`, {
      state: { propertyData: property, propertyId: id },
    });
  };
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            onClick={() =>
              navigate(
                id === 0
                  ? `/supply/supply-owner-table`
                  : `/supply/supply-property-form/${id}`
              )
            }
            type="button"
          >
              ...
              {loadingData ? (
                ...
                paginatedData.map((property, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-1.5 text-xs text-gray-800 max-w-[180px] truncate">
                      {formatter.format(
                        new Date(property?.submittedDateAndTime)
                      )}
                    </td>
                    <td className="px-3 py-1.5 text-xs text-gray-800 max-w-[180px] truncate">
                      {formatter.format(new Date(property?.updatedDateAndTime))}
                    </td>
                    <td className="px-3 py-1.5 text-xs text-gray-800">
                      <Eye
                        size={14}

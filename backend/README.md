...
├── stayease_sales/         # Tenants, rent, leads, Zoho e-sign
...
└── tenant_details/         # Tenant contract agreements
```
...
### `stayease_sales`
- **Tenant management**: Full tenant lifecycle — check-in, KYC, rent assignment, check-out
...
- **Liabilities**: Tenant deposit refund tracking
...
### `stayease_operations`
- **Move-in checklists**: Property condition assessment at tenant move-in
- **Move-in feedback**: Tenant satisfaction survey at check-in
- **Move-out checklists**: Property condition assessment at tenant departure
- **Move-out feedback**: Tenant experience survey at check-out
...
### `tenant_details`
- **Tenant contracts**: Identity verification documents (Aadhaar/PAN front/back copies)
...
| sales | `Tenant_Data` | supply | `Bed_Data` | ForeignKey |
| sales | `Tenant_Rent_Data` | sales | `Tenant_Data` | ForeignKey |
...
| accounts | `Liability_Detail` | sales | `Tenant_Data` | ForeignKey |
| accounts | `Rawdata_Detail` | supply | `Owner_Data` | ForeignKey |
| operations | `MoveInChecklistDetail` | sales | `Tenant_Data` | ForeignKey |
...
| `Bed_Data` | supply | Individual bed | bed label, room details, tenant info, check-in/out, rent, deposit, status |
| `Property_Detail` | supply | Website listing | images (6 slots), description, location, path, iframe link |
| `Tenant_Data` | sales | Active tenant | personal info, KYC, check-in/out, rent, deposit, status flags |
| `Tenant_Rent_Data` | sales | Monthly rent record | month, rent amount, delay charges, payment method, UTR, status |
...
| `Liability_Detail` | accounts | Deposit refund | tenant ref, amount, status, UTR, transfer date |
...
| `PropertyComplaintDetail` | operations | Complaint | tenant, description, preferred time |
...
| GET | `get-beds-data/` | Beds with tenant + liability info |
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `get-beds-data/` | Beds with tenant + rent records |
| POST | `tenant-form-submit/` | Create tenant (multipart) |
| PUT | `tenant-data-update/<id>/` | Update tenant (multipart) |
...
| GET | `/tenant-details/tenant-table/` | List tenant contracts |
| POST | `/tenant-details/tenant-data/` | Submit tenant contract |
...
stayease_sales           # Tenants, rent, leads, e-sign
